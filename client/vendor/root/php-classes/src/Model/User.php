...
class User extends Model {
    ...
    public function login($login,  $password)
    {
        ...
        $users = $sql->select("SELECT * FROM tb_clientes_usuarios WHERE usuario = :login", array(
            ":login" => $login
        ));
        ...
        if (isset($users[0]))
        {
            ...
            $verifyPass = $sql->select("SELECT * FROM tb_clientes_usuarios WHERE usuario = :login AND senha = :password", array(
                ":login" => $login,
                ":password" => $password
            ));
